...
export function RegisterPage(props: RegisterPageProps) {
  ...
  const schema = yup.object().shape({
    ...
    username: yup
      ...
      .test(
        "Mã người dùng có đủ 5 chữ số",
        "Mã người dùng phải bắt đầu bằng A và có 5 chữ số đằng sau",
        (values) => {
          return values.length === 6 && values[0] === "A"
        },
      )
      .matches(/^A\d{5}$/, "Mã người dùng không hợp lệ"),
    phoneNumber: yup
      .string()
  ...
  useEffect(() => {
    if (actionAuth == "Failed") {
      enqueueSnackbar("Mã người dùng đã được sử dụng", {
        variant: "error",
      })
    }
  }, [actionAuth])
          ...
          <FormProvider {...form}>
            ...
            >
              ...
              <Grid container spacing={2}>
                <Grid item xs={6}>
                  <InputField name="username" label="Mã người dùng" />
                </Grid>
                <Grid item xs={6}>
