...
import { Header, VoucherDesign } from "@/components/Common"
import {
  cartActions,
  iDataStore,
...
export default function Checkout(props: CheckoutProps) {
  ...
  const handleAddOrder = () => {
    if (sendTo.length <= 0) {
      enqueueSnackbar("Cần nhập địa chỉ", { variant: "error" })
      return
    }
    ...
    bill["paymentType"] = paymentMethod
    ...
    if (clientStomp) {
      clientStomp.subscribe(
        ...
        async (message) => {
          ...
          if (data.body.usernameSend === user?.username) {
            if (data.statusCodeValue === 400) {
              ...
              const newVoucher = data.body.data.vouchers as VoucherItem[]
              setVouchers(newVoucher)
              setOpenConfirm(false)
              if (data.body.data.bill.paymentType === "ONLINE") {
                const res = (await paymentApi.createPayment(
                  data.body.data.bill.id,
                  (data.body.data.bill.totalAmount + data.body.data.bill.shipFee),
                )) as any
  ...
  const [sendTo, setSendTo] = useState<string>("Đại học Thăng Long")
          ...
          >
            ...
            <DialogContent
              sx={{
                ...
                padding: `${
                  width > 450 ? "20px 24px !important" : "20px 10px !important"
                }`,
                overflow: "hidden auto",
              }}
            >
              {vouchers?.map((item) => (
                <VoucherDesign key={item.id} data={item} />
              ))}
            </DialogContent>
          </Dialog>
                ...
                <>
                  ...
                  >
                    <div
                      ...
                      className={`flex h-full gap-1 items-center justify-between ${
                        width > 500 ? "base-pd" : ""
                      }`}
                    >
