...
import { formatCurrencyVND, handleFoodBill } from "@/utils"
...
const BillItem = (props: BillItemProps) => {
  ...
  const renPayment = (status: "PAID" | "UNPAID") => {
    if (status === "PAID") {
  ...
  const renType = (status: "CASH" | "ONLINE") => {
    if (status === "CASH") {
    ...
    <>
      ...
      >
        ...
        >
          ...
          <div className="py-3">
            <p className="text-base font-semibold mb-2  ">Thanh toán</p>
            <p className="text-sm ">
              {renType(data.paymentType)} ({renPayment(data.paymentStatus)})
            </p>
          </div>
          <div className="py-2">
          ...
          <div className="py-2">
            <p className="text-base font-semibold mb-2">Đích đến</p>
            <p className=" text-sm">{data?.sendTo || "Không có"}</p>
          </div>
        </DialogContent>
